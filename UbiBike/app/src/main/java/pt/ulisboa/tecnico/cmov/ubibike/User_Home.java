...
public class User_Home extends AppCompatActivity  implements SimWifiP2pManager.PeerListListener, LocationListener {
    ...
    Runnable server = new Runnable() {
        ...
        public void run() {
            try {
                ...
                Log.d("Server. USER_HOME", messageFromServer);
                br.close();
                client.close();   //closing the connection
    ...
    */
    ...
    public void onPeersAvailable(SimWifiP2pDeviceList peers) {
        ...
        String station=station_aux.replace("_"," ");
        ...
        if (DataHolder.getInstance().isStationPastState() == false &&  DataHolder.getInstance().isBikePastState() == false && stationAtualState == true && bikeAtualState == true) {
            if (DataHolder.getInstance().getReservedBikes(station)==0){   //O NOME DA STATION TEM DE ESTAR IGUAL AO DA APLICAÇÃO
                message="Book_Bike"+","+station;
                //se nao ha bikes reservadas desconta uma bike no server
                Log.d("Management of bikes", message);
                communicateWithServer();
                if(messageFromServer.equals("This station doesn't have any bike available!")) {
                    Toast.makeText(User_Home.this, messageFromServer + "Please wait for an available bike.", Toast.LENGTH_SHORT).show(); //se nao tem bikes suficientes nao actualiza as variaveis
                    DataHolder.getInstance().setStationPastState(true);
                }
                else{
                    Toast.makeText(User_Home.this, "Arrived station and picked-up the bike: "+bike, Toast.LENGTH_SHORT).show();
                    DataHolder.getInstance().setStationPastState(true);
                    DataHolder.getInstance().setBikePastState(true);
                }
            }
            else{
                Toast.makeText(User_Home.this, "Arrived station and bike picked-up"+bike, Toast.LENGTH_SHORT).show();
                DataHolder.getInstance().setStationPastState(true);
                DataHolder.getInstance().setBikePastState(true);
            }
        }
        ...
        if (DataHolder.getInstance().isStationPastState() == true &&  DataHolder.getInstance().isBikePastState() == false && stationAtualState == true && bikeAtualState == true) {
            if (DataHolder.getInstance().getReservedBikes(station)==0){   //O NOME DA STATION TEM DE ESTAR IGUAL AO DA APLICAÇÃO
                message="Book_Bike"+","+station;                          //se nao ha bikes reservadas desconta uma bike no server
                Log.d("Management of bikes", message);
                communicateWithServer();
                if(messageFromServer.equals("This station doesn't have any bike available!"))
                    Toast.makeText(User_Home.this, messageFromServer+"Please wait for an available bike.", Toast.LENGTH_SHORT).show();
                else{
                    Toast.makeText(User_Home.this, "Arrived station and bike picked-up"+bike, Toast.LENGTH_SHORT).show();
                    DataHolder.getInstance().setStationPastState(true);
                    DataHolder.getInstance().setBikePastState(true);
        ...
        if (DataHolder.getInstance().isStationPastState() == false &&  DataHolder.getInstance().isBikePastState() == true && stationAtualState == false && bikeAtualState == false) {
            ...
            //CAPAZ DE FALTAR QQR COISA PARA AQUI
        }
        ...
        if (DataHolder.getInstance().isStationPastState() == false &&  DataHolder.getInstance().isBikePastState() == false && stationAtualState == false && bikeAtualState == true) {
            Toast.makeText(User_Home.this, "Bike picked-up anywhere!"+bike, Toast.LENGTH_SHORT).show();
            DataHolder.getInstance().setBikePastState(true);
        ...
        if (DataHolder.getInstance().isStationPastState() == true &&  DataHolder.getInstance().isBikePastState() == true && stationAtualState == true && bikeAtualState == false) {
            ...
            String distance = String.valueOf(haversine(firstLatitude,firstLongitude,lastLatitude,lastLongitude));
            String points = assignPointsToDistance(haversine(firstLatitude,firstLongitude,lastLatitude,lastLongitude));
            String messageToBuffer="Drop_Bike"+ ","+station;
            Log.d("Management of bikes", messageToBuffer);
            if (DataHolder.getInstance().getReservedBikes(station)==1)
                DataHolder.getInstance().dropReservedBike(station);
            messageToBuffer="Trajectory_Sender"+","+username+","+date+","+distance+","+points+","+station+","+coordinates.toString().substring(1,coordinates.toString().length()-1);
            Log.d("Message To Add Buffer", messageToBuffer);
            DataHolder.getInstance().addRequest(messageToBuffer);
            coordinates.clear();
        }
                ...
                try {
                    if (DataHolder.getInstance().bufferHaveSomeRequest()) {
                        ...
                        for (String aux : buffer_aux) {
                            ...
                            if(messageFromServer.contains("Points successfully updated.")) {
                                if(messageFromServer.contains("Receiver:")) {
                                    String[] points_aux = messageFromServer.split("Receiver:");
                                    String[] points_final = points_aux[1].split(" ");
                                    DataHolder.getInstance().updatePointsReceiver(Integer.parseInt(points_final[0]));
