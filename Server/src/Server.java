...
public class Server {
            ...
            try {
                ...
                switch(array[0]){
                    ...
                    case "Send_Points":
	                    ...
	                    if(array.length==7){
	                    	if(!usermap.get(array[1]).checkIfTransactionIsCommitted(Integer.parseInt(array[4]))){
                        		usermap.get(array[1]).addTransations(Integer.parseInt(array[4]));
	                        ...
	                        if(checkMessageIDController(array[0],array[3],array[4])){
		                        if(checkSignature(msgOriginal, array[5], array[1])){
		                        	if(!usermap.get(array[1]).checkIfTransactionIsCommitted(Integer.parseInt(array[4]))){
		                        		...
		                        		usermap.get(array[1]).sendPoints(Integer.parseInt(array[2]));
		                        		usermap.get(array[3]).receivePoints(Integer.parseInt(array[2]));
		                        		sendMessageToClient("Points successfully updated. Receiver:"+Integer.parseInt(array[2]));
		                        	}
		                        	else
		                        		sendMessageToClient("Points successfully updated. Receiver:"+Integer.parseInt(array[2]));
		                        }
                    ...
                    case "Station_Coord":
                        ...
                        for(String key: stationsmap.keySet()) {
                            if(stationsmap.get(key).getAvailableBikes()>0)
                                stc += key+";"+stationsmap.get(key).getLatitude() +";"+ stationsmap.get(key).getLongitude()+",";
                        }
                    ...
                    case "Trajectory_Sender":
                        System.out.println("Method: "+array[0]+" Username: "+array[1]+ " Date: "+array[2]+" Distance: "+array[3]+" Points: "+array[4]+" Station: "+array[5]);
                        ...
                        stationsmap.get(array[5]).dropBike();
    ...
    public static void populate(){
        stationsmap.put("Estação de Cascais", new Station("Cascais",15,38.70061,-9.41764, "Excellent"));
        stationsmap.put("Estação de São João do Estoril", new Station("São João do Estoril",15,38.70275, -9.39199, "Excellent"));
        stationsmap.put("Estação da Parede", new Station("Parede",12,38.68845, -9.36009, "Very Good"));
        stationsmap.put("Estação de Carcavelos", new Station("Carcavelos",10,38.68085, -9.33621, "Very Good"));
        stationsmap.put("Estação de Oeiras", new Station("Oeiras",10,38.68499, -9.31297, "Very Good"));
        stationsmap.put("Estação de Paço de Arcos", new Station("Paço de Arcos",8 , 38.69118, -9.29657, "Good"));  
